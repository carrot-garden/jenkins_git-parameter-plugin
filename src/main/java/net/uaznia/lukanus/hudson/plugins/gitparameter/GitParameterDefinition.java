...
public class GitParameterDefinition extends ParameterDefinition implements
	...
	public GitParameterDefinition(final String name, final String type,
	...
	public ParameterValue createValue(final StaplerRequest request) {
		final String value[] = request.getParameterValues(getName());
		if (value == null) {
	...
	public ParameterValue createValue(final StaplerRequest request,
		...
		if (value instanceof String) {
			...
			final JSONArray jsonValues = (JSONArray) value;
		...
		final GitParameterValue gitParameterValue = new GitParameterValue(
	...
	public void setBranch(final String nameOfBranch) {
	...
	public AbstractProject<?, ?> getParentProject() {
		...
		for (final AbstractProject<?, ?> project : jobs) {

			final ParametersDefinitionProperty property = project
					.getProperty(ParametersDefinitionProperty.class);

			if (property != null) {

				final List<ParameterDefinition> parameterDefinitions = property
	...
	@Override
	public int compareTo(final GitParameterDefinition pd) {
	...
	public void generateContents(final String contenttype) {
		...
		final GitSCM.DescriptorImpl desc = (GitSCM.DescriptorImpl) git
				.getDescriptor();
		...
		for (final RemoteConfig repository : git.getRepositories()) {
			for (final URIish remoteURL : repository.getURIs()) {
				...
				final IGitAPI newgit = new GitAPI(defaultGitExe,
				...
				if (type.equalsIgnoreCase(PARAMETER_TYPE_REVISION)) {
					...
					for (final ObjectId noid : oid) {

						final Revision r = new Revision(noid);
						final List<String> test3 = newgit.showRevision(r);
						final String[] authorDate = test3.get(3).split(">");
						final String author = authorDate[0].replaceFirst(
								"author ", "").replaceFirst("committer ", "")
								+ ">";
						String goodDate = null;
						try {
							final String totmp = authorDate[1].trim().split(
									"\\+")[0].trim();
							final long timestamp = Long.parseLong(totmp, 10) * 1000;
					...
					for (final String tagName : newgit.getTagNames("*")) {
